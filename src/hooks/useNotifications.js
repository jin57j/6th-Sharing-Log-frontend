...
async function requestNotifications(groupId) {
  const [summaryResponse, occurrences, requests] = await Promise.all([
  ...
  return {
    summary: summaryResponse ?? EMPTY_SUMMARY,
...
export default function useNotifications(groupId) {
  // 백엔드의 알림 개수 요약
    ...
    async (showLoading = true) => {
      ...
      try {
        ...
        const {
          ...
          occurrences,
          requests,
        } = await requestNotifications(groupId);
  ...
  async function handleSubstituteResponse(requestId, action) {
    ...
    const targetRequest = substituteRequests.find(
      (request) => request.requestId === requestId,
    );

    if (!targetRequest) {
      setErrorMessage("대타 요청 정보를 찾을 수 없습니다.");
    ...
    try {
      ...
      setErrorMessage(error.message ?? "대타 요청을 처리하지 못했습니다.");
    } finally {
  ...
  return {
    ...
    notificationCount: summary.unreadCount ?? 0,
